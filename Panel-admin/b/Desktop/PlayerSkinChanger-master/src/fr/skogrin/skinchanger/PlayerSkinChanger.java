...
public class PlayerSkinChanger extends JavaPlugin {
	...
	public final void onEnable() {
		try {
			...
			if (protocolLib != null
					&& !Utils.compareVersions("3.4.0", protocolLib.getDescription().getVersion().split("\\-")[0])) {
				playerDisplayModifier = new PlayerDisplayModifier(this);
				logger.log(Level.INFO, "ProtocolLib hooked !");
			} else {
				logger.log(Level.SEVERE,
						"Cannot hook into ProtocolLib because it is not running or you have installed a version older than 3.4.0 !");
